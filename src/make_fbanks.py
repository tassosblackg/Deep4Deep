...
def make_fb(train_phase,path):

    #train_phase = 'dev' # 'dev' 'eval'

    #base_dir = '/home/tassos/Desktop/DATA_ASR'
    base_dir=path
    norm_dir = os.path.join(base_dir, 'ASVspoof2017_V2_train_norm') #??

    if train_phase == 'train':
        wav_dir = os.path.join(base_dir, 'ASVspoof2017_V2_train') #from where to read data
        fbank_dir = os.path.join(base_dir, 'ASVspoof2017_V2_train_fbank') #where to save list
    ...
    frame_stride = 0.005
    NFFT = 2048
    nfilt = 64 # number of banks or windows on a time series

    #creates a list with each filename in a dir
...
def parseM():
    #menuDict={'train':,}
    print('RUN')
    parser=argparse.ArgumentParser(prog='make_fb')
    parser.add_argument('Settype',type=str,help="choose between train, dev, eval sets..")
    parser.add_argument('path',type=str,help='give path to where dataset are..')
    args=parser.parse_args()
    print(args)
    #call function
    make_fb(args.Settype,args.path)
